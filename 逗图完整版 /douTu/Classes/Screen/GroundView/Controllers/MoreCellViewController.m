...
@interface MoreCellViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout>
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath
{
    if(indexPath.section == 5){
        ...
        return CGSizeMake((SCREENWIDTH - 65) / 4, 20);
...
- (void)requestData
{
    if(self.model)
    {
        _str = [NSString stringWithFormat:@"http://api.jiefu.tv/app2/api/dt/item/getByTag.html?tagId=%@&pageNum=%d&pageSize=48",self.model.Id,_pageNum];
    }else
    {
        _str = [NSString stringWithFormat:@"http://api.jiefu.tv/app2/api/dt/item/getByTag.html?tagId=%@&pageNum=%d&pageSize=48",self.Id,_pageNum];
    }
    
    AFHTTPSessionManager * manager = [AFHTTPSessionManager manager];
    [manager GET:_str parameters:nil progress:^(NSProgress * _Nonnull downloadProgress)
     {
