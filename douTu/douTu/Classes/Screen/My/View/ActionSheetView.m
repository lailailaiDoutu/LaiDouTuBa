...
- (void)loadUiConfig
{
    ...
    for (NSInteger i = 0; i<_shareBtnImgArray.count; i++)
    {
        ...
        if (_shareBtnImgArray.count%3 == 0) {
            button.frame = CGRectMake(_backGroundView.bounds.size.width/3*(i%3), _LXActionSheetHeight+(i/3)*76 , _backGroundView.bounds.size.width/3 - 20, 85);
        }
        else
        {
            button.frame = CGRectMake(_backGroundView.bounds.size.width/4*(i%4), _LXActionSheetHeight+(i/4)*76, _backGroundView.bounds.size.width/4 - 20, 85);
        }
    ...
    for (NSInteger i = 0; i<_shareBtnTitleArray.count; i++)
    {
        ...
        if (_shareBtnTitleArray.count%3 == 0) {
            label.frame = CGRectMake(_backGroundView.bounds.size.width/3*(i%3), _LXActionSheetHeight+(i/3)*76 + 65, _backGroundView.bounds.size.width/3 - 20, 20);
        }
        else
        {
            label.frame = CGRectMake(_backGroundView.bounds.size.width/4*(i%4), _LXActionSheetHeight+(i/4)*76 + 65, _backGroundView.bounds.size.width/4 - 20, 20);
        }
        
        label.text = _shareBtnTitleArray[i];
        label.textAlignment = NSTextAlignmentCenter;
        label.font = [UIFont systemFontOfSize:14];
        [self.topsheetView addSubview:label];
    }
...
- (UIView *)backGroundView
{
    if (_backGroundView == nil) {
        ...
        {
            _backGroundView.frame = CGRectMake(0, ActionSheetH, ActionSheetW, _shareBtnTitleArray.count*50+50+7+(_protext.length==0?0:45)+20);
